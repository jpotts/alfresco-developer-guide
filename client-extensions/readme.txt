...
MPXJ was upgraded to HEAD (3.2.0) in this version of the source to be compatible with POI 3.5.

In later versions, Alfresco included an MS Project mime type. So in this version of the source, the new mime type definition is commented out.
...
Starting with Alfresco 3.2 Enterprise, Guest appears to no longer be granted access to services secured with ROLE_AUTHENTICATED. The security bean in this version of the source has been changed to ACL_ALLOW.
...
Chapter 8 is a light introduction to Alfresco WCM from a developer's perspective. It includes a basic web form, an ASR callback, and a "no approval" workflow. The chapter also shows you how to deploy code to a test server from a web script using the AVM API.

Starting with Alfresco 3.2 Enterprise, when working with AVM Nodes in Freemarker, properties in the "sys" namespace are no longer included in the properties hash. The RSS template for press releases has been updated to handle the change.

Chapter 9 Examples

Chapter 9 is on authentication and authorization. It includes using LDAP for authentication, setting up chaining, creating a custom role, and implementing single sign-on with CAS.

This chapter depends on the "someco-server-extensions" project.
